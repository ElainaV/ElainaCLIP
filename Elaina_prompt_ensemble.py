...
class AnomalyCLIP_PromptLearner(nn.Module):
    def __init__(self, clip_model, design_details):
        ...
        self.depth        = design_details.get("learnabel_text_embedding_depth", 1)   # 【NEW‑1】
        self.text_n_ctx   = design_details.get("learnabel_text_embedding_length", 4) # 【NEW‑2】
    ...
    @torch.no_grad()
    def register_fixed_embedding(self, img_emb: torch.Tensor):
        assert img_emb.dim() == 2 and img_emb.size(0) == 1, "register_fixed_embedding, shape=[1,768]"
        self.fixed_image_embedding = img_emb.detach().clone()
        print("[DynPrompt] Fixed image embedding registered.")

    def forward(self, cls_id=None, image_embedding=None):
        ctx_dim = self.ctx_pos.shape[-1]

        if self.training:
            if not hasattr(self, "fixed_image_embedding"):
                raise RuntimeError("register_fixed_embedding()")
            dyn_in = self.fixed_image_embedding
        else:
            if image_embedding is None or image_embedding.size(0) != 1:
                raise RuntimeError("batch_size==1, image_embedding")
        ...
        print("[Assemble] ctx_pos", tuple(ctx_pos.shape))
        print("[Assemble] ctx_neg", tuple(ctx_neg.shape))
        ...
        return prompts, token_ids, self.compound_prompts_text  # 【NEW‑6】
